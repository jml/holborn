...
in
{
  ...
  config = {
    ...
    systemd.services."holborn-api" = {
      ...
      serviceConfig.ExecStart = ''${cfg.package}/bin/holborn-api-server --port=${toString cfg.port} --postgres-database="holborn" --postgres-user="holborn" --base-url="https://norf.co" --repo-hostname="${cfg.repoServer}" --repo-http-port=${toString cfg.repoPort}  --repo-git-port=${toString cfg.rawRepoPort}'';
